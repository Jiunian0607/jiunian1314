...
window.simulateReply = function() {
    ...
    // ── 反向提问检查 ──
    if (typeof window.askMeChance === 'function' && Math.random() < window.askMeChance('partner')) {
const defaultQuestions = ['今天心情怎么样？', '想我了吗？', '你觉得呢？', '要不要一起做什么？', '今天过得开心吗？', '有没有想对我说的话？'];
const pool = (customReverseQuestions && customReverseQuestions.length > 0) ? customReverseQuestions : defaultQuestions;
const q = pool[Math.floor(Math.random() * pool.length)];
        if (typeof window.showReverseQuestion === 'function') {
            window.showReverseQuestion(settings.partnerName || '梦角', q, 'partner', function(answer) {
                addMessage({
                    id: Date.now(),
