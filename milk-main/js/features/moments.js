...
async function saveMoment(data) {
    try {
        var list = await getAll();  // 使用 getAll 保持一致性
...
async function getByMode(mode) {
    ...
    function buildMomentCard(m) {
        ...
        // 媒体图片
        var mediaHtml = '';
        if (m.media && m.media.length > 100) {
            var safeSrc = m.media.replace(/'/g, "\\'");
            mediaHtml = '<img src="' + m.media + '" class="moment-media" loading="lazy" onclick="window.openImageViewer && window.openImageViewer(\'' + safeSrc + '\')" style="cursor:pointer;max-width:100%;max-height:300px;border-radius:12px;margin-top:8px;" onerror="this.style.display=\'none\'">';
        }
